...
export function LeafletMap({ points, polygons, startPoints, onMapReady, onPointClick }: LeafletMapProps) {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current, {
      zoomControl: true,
      attributionControl: false,     // 🟢 ПОЛНОСТЬЮ УБРАЛИ НАДПИСЬ
      preferCanvas: true,            // 🟢 быстрее для 3000 точек
      fadeAnimation: false,
      zoomAnimation: false,
    ...
    const osm = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      ...
      // ⚡ УСКОРЕНИЕ ЗАГРУЗКИ
      detectRetina: false,     // в 2 раза меньше тайлов
      keepBuffer: 2,          // меньше памяти
      updateWhenIdle: true,   // грузим только после движения
    });
